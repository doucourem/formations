...
const fetchCashesAndTransactions = useCallback(async () => {
  ...
  try {
    ...
    if (profile?.role === "kiosque" || profile?.role === "grossiste") {
      ...
      // 🔹 Admin : récupérer seulement ses kiosques
      const { data: myKiosks, error: kiosksError } = await supabase
        .from("kiosks")
        .select("id")
        .eq("owner_id", user.id);

      if (kiosksError) throw kiosksError;
      const kioskIds = (myKiosks || []).map(k => k.id);
      ...
      // Autres rôles : toutes les caisses
    ...
    // 4️⃣ Récupérer transactions pour ces caisses
    ...
    // 5️⃣ Calcul des soldes après transaction et enrichissement
    const enriched = txData.map((t) => {
      ...
      return {
        ...
        below_min: cash ? t.amount >= cash.min_balance : false,
      };
    });

    setTransactions(enriched);

    // 6️⃣ Pré-sélection si nécessaire
