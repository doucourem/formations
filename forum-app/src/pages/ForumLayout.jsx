      ...
      {forumData.map((section, index) => (
        <div key={index} className="mb-12">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm bg-gray-900 rounded-lg overflow-hidden">
              <thead className="bg-gray-800 text-gray-300">
                <tr>
                  <th className="text-left p-3">Category</th>
                  <th className="text-left p-3">Description</th>
                  <th className="text-center p-3">Topics</th>
                  <th className="text-center p-3">Posts</th>
                  <th className="text-left p-3">Last Post</th>
                </tr>
              </thead>
              <tbody>
                {section.categories.map((cat, idx) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-gray-300">
                      <div className="text-blue-300">{cat.lastPost.title}</div>
                      <div className="text-xs text-gray-400">{cat.lastPost.time}</div>
                      <div className="text-xs text-green-400">{cat.lastPost.author}</div>
                    </td>
                  </tr>
                ))}
              </tbody>
