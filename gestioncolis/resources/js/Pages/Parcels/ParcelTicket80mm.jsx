...
export default function ParcelTicket80mm({ parcel }) {
...
const STATUS_FR = {
  ...
  const numberToWordsFR = (n) => {
    ...
    const conv = (x) => {
      ...
      if (x < 70) return d[Math.floor(x/10)] + (x%10 ? "-" + u[x%10] : "");
      if (x < 80) return "soixante-" + conv(x-60);
      return "quatre-vingt" + (x%20 ? "-" + conv(x-80) : "");
    };
    let r = "";
    if (n >= 1000) { r += conv(Math.floor(n/1000)) + " mille "; n %= 1000; }
  ...
  const generateTicket = async () => {
    ...
    doc.text(`Tracking : ${parcel.tracking_number || parcel.id}`, 5, y); y += 6;
    doc.line(5, y, 75, y); y += 5;

    // Expéditeur
    doc.setFont("helvetica", "bold"); doc.text("EXPÉDITEUR", 5, y); y += 4;
    doc.setFont("helvetica", "normal"); doc.text(parcel.sender_name || "-", 5, y); y += 4;
    doc.setFontSize(8); doc.text(`Agence : ${parcel.senderAgency?.name || "-"}`, 5, y); y += 6;

    // Destinataire
    doc.setFont("helvetica", "bold"); doc.text("DESTINATAIRE", 5, y); y += 4;
    doc.setFont("helvetica", "normal"); doc.text(parcel.recipient_name || "-", 5, y); y += 4;
    doc.setFontSize(8); doc.text(`Agence : ${parcel.recipientAgency?.name || "-"}`, 5, y); y += 6;

    doc.setFontSize(9); doc.line(5, y, 75, y); y += 6;
    doc.text(`Poids : ${parcel.weight_kg ?? "-"} kg`, 5, y); y += 5;
    doc.text(`Statut : ${STATUS_FR[parcel.status] || "-"}`, 5, y); 
y += 6;


    // Montant
    doc.setFont("courier", "bold"); doc.setFontSize(12);
    const text = `${amount.toLocaleString("fr-FR")} FCFA`;
    ...
    doc.addImage(qr, "PNG", 25, y, 30, 30); y += 34;
    doc.setFontSize(7); doc.text("Scanner pour suivi", center, y, { align: "center" }); y += 6;
