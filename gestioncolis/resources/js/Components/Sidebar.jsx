...
export default function Sidebar({ onNavigate }) {
    ...
    const navigate = (routeName) => {
        ...
        if (onNavigate) onNavigate(); // 🔹 ferme le drawer mobile après clic
    };
    ...
    return (
        ...
        >
            ...
            <Divider />
            <List sx={{ px: 2 }}>
                <ListItemButton onClick={() => router.post(route('logout'))} sx={{ borderRadius: 2, color: 'error.main' }}>
                    <ListItemIcon sx={{ minWidth: 40, color: 'error.main' }}>
                        <LogoutIcon />
                    </ListItemIcon>
                    <ListItemText primary="Déconnexion" primaryTypographyProps={{ fontSize: '0.85rem', fontWeight: 600 }} />
                </ListItemButton>
            </List>
        </Box>
    );
