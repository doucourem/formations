...
export default function UsersList() {
  ...
  const [currentUser, setCurrentUser] = useState(null); // 🔹 état user connecté
  ...
  // 🔹 Fetch des utilisateurs dès que currentUser est disponible
  useEffect(() => {
    if (currentUser) fetchUsers();
  }, [currentUser]);

  // 🔹 Récupération des utilisateurs appartenant au currentUser
  const fetchUsers = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq("owner_id", currentUser.id) // seulement ses utilisateurs
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingUser) {
        ...
        // Insertion dans table users avec owner_id
